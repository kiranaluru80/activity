			...
			using (var reader = new System.IO.StreamReader(stream))
			{
				...
				for (int i = 0; i < data.Count; i++)
				{
                    ...
                    tapGestureRecognizer.Tapped += (object sender, EventArgs e) => {
                        StackLayout selectedStack = (StackLayout)sender;
                        IList<View> stackLayoutChilders = subStack.Children;
                        var childStack = stackLayoutChilders[1];
                        if (childStack.IsVisible){
                            childStack.IsVisible = false;
                        }else{
                            childStack.IsVisible = true;
                        }
                    };
					subStackChaid.GestureRecognizers.Add(tapGestureRecognizer);
                    ...
                    visibleOrInvisibleStack.IsVisible = false;
